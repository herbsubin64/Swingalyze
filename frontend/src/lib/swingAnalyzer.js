/**
 * Lightweight, client-side Swingalyzer:
 *  - Detects Address → Top → Impact
 *  - Computes tempo & X-factor
 *  - Flags faults (head sway proxy, early extension, loss of posture, OTT proxy)
 *  - Returns suggested drills & coaching tips per fault
 */
...
export function emptyAnalysis(){
  return { frames:[], events:{address:null,top:null,impact:null}, faults:[], summary:{frames:0,fps:0,tempo:{backMs:0,downMs:0,ratio:0}, xFactorDeg:0} }
}
...
function phaseDetect(prev, f){
  ...
  // Address: ~200ms after start (stable)
  if (!events.address && (t - frames[0].tMs) > 200) events.address = { tMs: t-200, idx: Math.max(0, frames.length-2) }

  // Top: when lead wrist vertical velocity flips from up to down after rising a bit
  ...
  // Impact: local min of wrist-hip distance after Top
  ...
  // Early extension: knees straighten from Top → Impact
  if (events.top){
    ...
    if (kTop && kImp){
      ...
      if (dK > 12) faults.push(fault('Early Extension', 'bad', kImp.tMs, 'Downswing',
        `Knees straightened ~${Math.round(dK)}° from top → impact.`,
        drillsEarlyExtension()))
    }
  }

  // Loss of posture: spine tilt change from Address → Impact
  if (events.address){
    ...
    const dS = sImp - sAddr
    if (Math.abs(dS) > 10) faults.push(fault('Loss of Posture', 'warn', events.impact?.tMs ?? frames.at(-1).tMs, 'Impact',
      `Spine tilt changed ~${Math.round(dS)}°.`, drillsPosture()))
  }

  // Over-the-top proxy: downswing hand path vs shoulder line
  if (events.top && frames.length>events.top.idx+6){
    ...
    if (Math.abs(diff) > 35) faults.push(fault('Over-the-Top (proxy)', 'warn', b.tMs, 'Downswing',
      `Lead hand path deviates ~${Math.round(diff)}° vs shoulder line.`, drillsOTT()))
  }

  // X-factor range @ Top
  if (events.top){
    const xf = summary.xFactorDeg
    if (xf < 15) faults.push(fault('Limited X-Factor', 'warn', events.top.tMs, 'Top',
      `Separation only ${Math.round(xf)}° (power leak).`, drillsXFactorLow()))
    if (xf > 60) faults.push(fault('Over-torque (X-Factor)', 'warn', events.top.tMs, 'Top',
      `Separation ~${Math.round(xf)}°; may hurt consistency.`, drillsXFactorHigh()))
  }

  // Head/lateral sway proxy (range of head X)
  if (frames.length>6){
    const xs = frames.map(fr=>fr.head.x), sway = Math.max(...xs)-Math.min(...xs)
    if (sway > 30) faults.push(fault('Excess Head Sway', 'warn', events.top?.tMs ?? frames.at(-1).tMs, 'Backswing',
      `Head moved laterally ~${Math.round(sway)}px; keep it steadier.`, drillsHeadSway()))
  }
...
function fault(title, severity, timeMs, phase, detail, drills){
  return { title, severity, timeMs, phase, detail, drills }
}

/** Suggested drills & tips */
function drillsEarlyExtension(){ return [
  { name:'Chair/Wall Butt Drill', tip:'Set your rear against a chair/wall at address; keep contact through impact.' },
  { name:'Pump Drill (Half Swings)', tip:'Pause at top, make small "pump" moves keeping knee flex, then swing.' },
  { name:'Stick Through Belt Loops', tip:'Align a stick across hips; keep tush line back as you rotate.' }
]}
function drillsPosture(){ return [
  { name:'Club Across Chest Tilt', tip:'Feel lead-shoulder down toward ball in backswing; maintain tilt into impact.' },
  { name:'Alignment Rod Down Spine', tip:'Keep constant distance from rod to ground during swing.' }
]}
function drillsOTT(){ return [
  { name:'Trail Arm Underhand Toss', tip:'From top, toss a ball side-arm to right field (RH golfer) to feel inside path.' },
  { name:'Headcover Outside Ball', tip:'Place headcover just outside ball line; avoid hitting it on downswing.' }
]}
function drillsXFactorLow(){ return [
  { name:'Shoulders Turn, Hips Quiet', tip:'Cross arms on chest; turn shoulders to 90° while keeping hips quieter.' },
  { name:'Resistance Band Top', tip:'Anchor band to hips, rotate shoulders against resistance to feel separation.' }
]}
function drillsXFactorHigh(){ return [
  { name:'Shorter Backswing', tip:'Stop when lead arm is parallel to ground; keep tempo smooth.' },
  { name:'Match Arm-Body Pace', tip:'Focus on connected arms with torso; avoid over-cranking shoulders.' }
]}
function drillsHeadSway(){ return [
  { name:'Head Against Towel/Wall', tip:'Lightly keep head near a soft reference on backswing.' },
  { name:'Feet Together Swings', tip:'Make half swings feet together to promote centered turn.' }
]}